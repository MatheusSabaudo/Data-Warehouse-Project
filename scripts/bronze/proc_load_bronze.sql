...
Purpose: This procedure performs the initial data load into the Bronze layer
         of the data warehouse. It truncates and reloads all tables in the 
         bronze schema by importing data from CSV source files from both CRM 
         and ERP systems.

Process Flow:
         1. Load CRM source tables (crm_cust_info, crm_prd_info, crm_sales_details)
         2. Load ERP source tables (erp_cust_az12, erp_loc_a101, erp_px_cat_g1v2)
         3. Each load operation includes truncation before bulk insert
         4. Performance metrics are captured and displayed for each load
